...
import {convertRawIntToRupiah, convertToApiDate} from '../../../utils/Helper'
...
onMounted(async()=>{
    const res = await executionStore.getExecutionByID(execID)
    if(res != null){
        if(res.responseCode == "00" && res.responseData != null) {
            data.value = res.responseData
            executerLabel.value = res.responseData.executer_name
            carLabel.value = res.responseData.plat
            customerLabel.value = res.responseData.customer_name
            if(res.responseData.paid_off != null){
                data.value.paid_off = res.responseData.paid_off? 1:0
            }
            data.value.paid_at = convertToApiDate(res.responseData.paid_at)
            data.value.executed_at = convertToApiDate(res.responseData.executed_at)
            data.value.finish_at = convertToApiDate(res.responseData.finish_at)
        }
    }
})
...
const updateData = async() => {
  const res = await executionStore.updateExecution(data.value)
  if(res != null){
    if(res.responseCode == "00") {
        alert("success update execution")
